...
void timer1ToDisplay(byte x, byte y) {         //выводит состояние таймера=>
  ...
  if (timer1_off == 1) {
    myOLED.print(TIMER_OFF); //"ТАЙМ.ВЫКЛ"
  } else {
    //myOLED.setFont(SmallFontRus);
    //вывод таймера
    if (timer1_h < 10) {
...
void timeToDisplay (byte x, byte y, bool showSeconds) {
  DateTime now = rtc.now();
  /*if (showSeconds == 1) {
    myOLED.setFont(MediumFontRus);       //установка шрифта BigNumbers
    } else {
    myOLED.setFont(MegaNumbers);
    }*/
  myOLED.setCursor(x, y);           //это нижняя строка         //вывод на экран: myOLED.setCursor(3,7);(лево-право,верх-низ)
  if (now.hour() < 10) {
    myOLED.print(0);
  } myOLED.print(now.hour());       //часов
  myOLED.print(".");                //"."
  if (now.minute() < 10) {
    myOLED.print(0);
  } myOLED.print(now.minute());     //минут
  if (showSeconds == 1) {
    myOLED.print(".");
    myOLED.setFont(MediumFontRus);
    if (now.second() < 10) {
    ...
    myOLED.print(now.second());     //секунд
  }
}
//============================================================================
void timeToDisplay1() {// вывод на эран верхняя строка:день недели число.месяц.год шрифт MediumFontRus (средний)
  ...
  if (showTemp == 1) {
    dataToDisplay (9, 3); //вызов отображения даты на дисплей (byte x, byte y)
  } else {
    //(showTemp == 0)  
    tempAndHumidToDisplay(3, 3);
  }
  myOLED.setFont(BigNumbers);
...
//======================                          //======================
//============================================================================
void timeToDisplay2() {// вывод на эран верхняя строка: день недели-число.месяц.год шрифт MediumFontRus (средний)
  ...
  if (showTemp == 1) {
    ...
    dataToDisplay (9, 2); //вызов отображения даты на дисплей (byte x, byte y)
   }
  myOLED.setFont(MegaNumbers);
  timeToDisplay (6, 7, 0);
}
...
void diagnosticsToDisplay() {
  ...
  if (showTemp == 1){
  tempAndHumidToDisplay(0, 0);
  } else {
  myOLED.print(F("Темп. RTC "), 0, 0); myOLED.print(rtc.getTemperature()); myOLED.print(F(" C"));
  }
  //myOLED.print(F("Темп. "));
  //======================================
  myOLED.setFont(MediumFontRus);
